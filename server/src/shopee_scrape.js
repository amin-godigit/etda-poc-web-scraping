...
const { signInShopee, moveMouseNaturally } = require("./utils");
const fs = require("fs");
...
const fetchCategories = async () => {
  try {
    const browser = await puppeteer.launch({
      executablePath:
        "C:/Users/Hero/AppData/Local/Google/Chrome/Application/chrome.exe",
      userDataDir: "C:/Users/Hero/AppData/Local/Google/Chrome/User Data",
      headless: false,
      args: [
    ...
    await client.send("Page.addScriptToEvaluateOnNewDocument", {
      source: `
        ...
        Object.defineProperty(navigator, 'connection', {
          get: () => ({
            effectiveType: '4g',
            rtt: 50 + Math.floor(Math.random() * 40),
            downlink: 10.0 + Math.random() * 2,
            saveData: false
          })
        });
    ...
    await page.evaluateOnNewDocument(() => {
      ...
      performance.now = () =>
        originalPerformanceNow.call(performance) + Math.random() * 10;
    ...
    const savedCookies = JSON.parse(
      await fs.promises.readFile("./src/data/cookies.json")
    );

    // กรอง cookies ที่มี name
    const validCookies = savedCookies.filter((cookie) => {
    ...
    // เพิ่ม domain ถ้าขาด
    const cookiesWithDomain = validCookies.map((cookie) => {
      if (!cookie.domain && !cookie.url) {
        return { ...cookie, domain: ".shopee.co.th" };
      }
      return cookie;
    });

    // ลบ cookies เดิมก่อนตั้งค่าใหม่
    const currentCookies = await page.cookies();
    const validCurrentCookies = currentCookies.filter(
      (cookie) => cookie.name && typeof cookie.name === "string"
    );
    await page.deleteCookie(...validCurrentCookies);
    ...
    // $(".image-carousel__item-list-wrapper > ul").each((index, element) => {
    //   const title = $(element)
    //     .find("li > a > div > div:nth-child(2) > div")
    //     .trim();
    //   const url = $(element).find("li > a").attr("href") || "";
    //   const category = {
    //     name: title,
